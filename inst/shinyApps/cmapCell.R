...
#shiny
########################
cmapCellFilter = function(cellDataFile,oboFile=NA) {
  #Setup:
    cmapCelldata<<-read.csv(cellDataFile,sep="\t")
    origdata<<-cmapCelldata
...
########################
  ...
  mylog=c()
    ...
    shinyApp(
        ...
        ##########
        ui = fluidPage(
            titlePanel("cmapCellFilter"),
            sidebarLayout(position = "left",
                  sidebarPanel(width=2,
                    selectInput("filterVar","Filter Column",names(cmapCelldata)),
                    selectInput("filterRel","Relation",relations),
                    textInput("filterPattern", "Filter Pattern"),
                    selectInput("plotVar","Plot Column",names(cmapCelldata)),
        ...
        server = function(input, output, session) {
         ...
         observeEvent(input$filterButton, {
               #subs=filterForage(cmapCelldata,input$filterVar,input$filterPattern)
               subs=relFunc(cmapCelldata,input$filterVar,input$filterRel,input$filterPattern)
               #table
               ...
               cmapCelldata<<-subs[[1]]
               #logtext=paste("cmapCelldata", input$filterVar, input$filterRel, input$filterPattern)
               logtext="inInd=NA"
               mylog<<-c(mylog,logtext)
               logtext=relTextFunc(relText[[input$filterRel]],cmapCelldata,input$filterVar,input$filterPattern,input$filterRel)
               mylog<<-c(mylog,logtext)
               logtext="cmapCelldata=cmapCelldata[inInd,]"
               mylog<<-c(mylog,logtext)
               output$myLog=renderUI({HTML(paste(mylog,collapse="<br>"))})
          })

          observeEvent(input$resetButton, {
            cmapCelldata<<-origdata
            output$mytable = renderDataTable({cmapCelldata},options = list(scrollX = TRUE,pageLength=5))
          ...
          output$downloadData <- downloadHandler(
             ...
             content = function(file) {
                 write.csv(cmapCelldata, file,row.names=F)
             }
          )
